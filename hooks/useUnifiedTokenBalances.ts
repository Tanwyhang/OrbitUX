...
/**
 * Token Balance Hook (Unified)
 * Works with unified config for both Uniswap and Stargate
 */
...
export function useUnifiedTokenBalances(): UseTokenBalancesResult {
  const { address, isConnected } = useAccount();
  const chainId = useChainId();
  const publicClient = usePublicClient();

  // Only fetch balances if on a supported chain
  const isSupportedChain = Object.values(SUPPORTED_CHAINS).includes(chainId as any);
  ...
  const fetchBalances = useCallback(async () => {
    ...
    // Skip fetching if on unsupported chain (token addresses won't match)
    if (!Object.values(SUPPORTED_CHAINS).includes(chainId as any)) {
      const empty: Record<string, bigint> = {};
      tokenList.forEach(t => empty[t.symbol] = ZERO);
      setBalances(empty);
      return;
  ...
  }, [address, isConnected, publicClient, tokenList, chainId]);
